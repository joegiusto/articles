...
function AdminOrders() {
    ...
    useEffect(() => {
        ...
        .then(function (response) {
            ...
            let alteredProducts = response.data.map(product => {
                product.quantity = 0
    ...
    const addItem = () => {
		...
		axios.post('/api/admin/orders/add', {
			order
		})
		.then( (response) => {
			console.log(response)
                    ...
                    <div className="donate-form">
                        ...
                        <div className="row">
                            ...
                            <div className="col-lg-6">
                                ...
                                <div className="form-group articles">
                                    <label className="d-flex justify-content-between" htmlFor="order.payment.total">
                                        <span>Amount</span>
                                        <span>${ numberWithCommas( (order.payment?.total / 100).toFixed(2) ) }</span>
                                    </label>
                                    <input 
                                        className="form-control with-label"
                                        onChange={handleFieldChange}
                                        name="order.payment.total"
                                        id="order.payment.total"
                                        type="text" 
                            ...
                            <div className="col-lg-6">
                                ...
                                {order.user_id?._id && 
                                    <div className="w-100 d-flex justify-content-center align-items-center h-100">
                                        <div className="mb-3"><AdminViewUserModal user_id={order.user_id._id} name={`${order.user_id.first_name} ${order.user_id.last_name}`} buttonType={'badge'} /></div>
                                    </div>
                                }
                            ...
                            <div className="col-lg-12 mb-3">
                                ...
                                <div>
                                    <div onClick={() => handleFieldChange({ target: {name: 'status', value: 'Awaiting Shipment'} } )} className={`btn btn-articles-light btn-sm ${order.status == 'Awaiting Shipment' && 'active'} `}>
                                        Awaiting Shipment
                                    </div>
                                    <div onClick={() => handleFieldChange({ target: {name: 'status', value: 'Shipped'} } )} className={`btn btn-articles-light btn-sm ${order.status == 'Shipped' && 'active'} `}>
                                        Shipped
                                    </div>
                                    <div onClick={() => handleFieldChange({ target: {name: 'status', value: 'Delivered'} } )} className={`btn btn-articles-light btn-sm ${order.status == 'Delivered' && 'active'} `}>
                                        Delivered
                                    </div>
                                </div>
                            ...
                            <div className="col-lg-12 mb-3">
                                ...
                                <div className="form-group articles">
                                    <label htmlFor="address">Product Search</label>
                                    <input 
                                        className="form-control with-label"
                                        onChange={handleFieldChange}
                                        name="productSearch" 
                                        id="productSearch" 
                                        type="text" 
                                        value={order.productSearch}
                                    />
                                </div>
                                
                                <div style={{fontSize: '0.8rem'}} className="px-2">
                                    ...
                                    {/* {products.map(product => 
                                        <div key={product._id} className="d-flex justify-content-between mb-2">
                                            ...
                                            <div>
                                                <button disabled={product.quantity == 0 && true} onClick={ () => changeQuantity(product._id, -1) } className="btn btn-articles-light btn-sm"><i className="fad fa-minus mr-0"></i></button>
                                                <span style={{width: '30px'}} className="badge badge-light border border-dark mx-2">{product.quantity}</span>
                                                <button onClick={ () => changeQuantity(product._id, 1) } className="btn btn-articles-light btn-sm ml-2"><i className="fad fa-plus mr-0"></i></button>
                                            </div>
                                        </div>
                                    )} */}

                                    {order.products?.map(product => 
                                        <div key={product._id} className="d-flex justify-content-between align-items-center mb-2 border-bottom border-dark">
            ...
            <div className="side-panel">
                ...
                <div className="card">
                    ...
                    <div className="card-body">
                        {/* <div>Preorders - 0</div> */}
                        <div>Awaiting Shipment - {orders.filter(order => order.status === "Awaiting Shipment").length}</div>
                        <div>Shipped - {orders.filter(order => order.status === "Shipped").length}</div>
                        <div>Delivered - {orders.filter(order => order.status === "Delivered").length}</div>
                        <hr />
                        <div>Deleted - 0</div>
                    </div>
                </div>
            ...
            <div className="main-panel orders">
                ...
                <div className="card manage-card mb-3">
                    ...
                    <div className="card-header flex-column">
                        ...
                        <div className="d-block w-100 table-filters d-flex mt-2 mt-lg-3">
                                <button onClick={() => set_table_tab('Awaiting Shipment') } className={"btn btn-articles-light btn-sm d-flex flex-grow-1 " + (table_tab === 'Awaiting Shipment' ? 'alt' : '')}>Awaiting Shipment ({orders.filter(order => order.status === 'Awaiting Shipment').length})</button>
                                <button onClick={() => set_table_tab('Shipped') } className={"btn btn-articles-light btn-sm ml-1 d-flex flex-grow-1 " + (table_tab === 'Shipped' ? 'alt' : '')}>Shipped ({orders.filter(order => order.status === 'Shipped').length})</button>
                                <button onClick={() => set_table_tab('Delivered') } className={"btn btn-articles-light btn-sm ml-1 d-flex flex-grow-1 " + (table_tab === 'Delivered' ? 'alt' : '')}>Delivered ({orders.filter(order => order.status === 'Delivered').length})</button>
            ...
            <div className="admin-page admin-orders">
                ...
                <div className={"tracking-code-popup " + (tracking_code_popup ? 'active' : '')}>
                    ...
                    <div className="window">
                        ...
                        <div className="form-group articles">
                            <label htmlFor="title">Tracking Code</label>
                            <input
                                type="text"
                                className="form-control with-label"
                                value={tracking_code}
